...
class PRM:
    ...
    def uniform_sample(self, n_pts):
        ...
        n_sample_row = int(np.sqrt(n_pts * self.size_row / self.size_col))
        n_sample_col = int(n_pts / n_sample_row)
        sample_row = np.linspace(0, self.size_row-1, n_sample_row, dtype=int)
        sample_col = np.linspace(0, self.size_col-1, n_sample_col, dtype=int)
    ...
    def random_sample(self, n_pts):
        ...
        # Initialize graph
        self.graph.clear()
        sample_row = np.random.randint(0, self.size_row-1, n_pts, dtype=int)
    ...
    def gaussian_sample(self, n_pts):
        ...
        sample_row = np.random.randint(0, self.size_row-1, n_pts, dtype=int)
        sample_col = np.random.randint(0, self.size_col-1, n_pts, dtype=int)
        g_row = sample_row + np.random.normal(0.0, 10, n_pts).astype(int)
        g_col = sample_col + np.random.normal(0.0, 10, n_pts).astype(int)
        for row1, col1, row2, col2 in zip(sample_row, sample_col, g_row, g_col):
    ...
    def bridge_sample(self, n_pts):
        ...
        for row1, col1 in zip(sample_row, sample_col):
            ...
            if 0 <= col1 < self.size_col and 0 <= row1 < self.size_row:
                if self.map_array[row1][col1] == 0:
                    g_row = int(np.random.normal(row1, 30, 1))
                    g_col = int(np.random.normal(col1, 30, 1))
                    if 0 <= g_col < self.size_col and 0 <= g_row < self.size_row:
                        if self.map_array[g_row][g_col] == 0:
                            # check for free space point -Mid point 
                            mid_row, mid_col = int(0.5*(row1+g_row)), int(0.5*(col1+g_col))
    ...
    def search(self, start, goal):
        ...
        #start_pairs = ((('start', neighbor,self.dis(self.samples[neighbor],start)) for neighbor in neighbors[0]))
        #goal_pairs = ((('goal', neighbor,self.dis(self.samples[neighbor],goal)) for neighbor in neighbors[1]))
        start_pairs=[]
        goal_pairs=[]
