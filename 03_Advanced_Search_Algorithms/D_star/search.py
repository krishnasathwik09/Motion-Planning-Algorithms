...
class DStar:
    ...
    def process_state(self):
        ...
        if k < node_m.h:
            ...
            for nbr in self.get_neighbors(node_m):
                if nbr.h <= k and node_m.h > nbr.h + self.cost(nbr, node_m):
                    node_m.parent = nbr
                    node_m.h = nbr.h + self.cost(nbr, node_m)
        # If node k is the same as h (LOWER)           
        if k == node_m.h:
            for nbr in self.get_neighbors(node_m):
                if nbr.tag == 'NEW' or (nbr.parent == node_m and nbr.h != node_m.h + self.cost(node_m, nbr)) or (nbr.parent != node_m and nbr.h > node_m.h + self.cost(node_m, nbr)): 
                    nbr.parent = node_m
                    self.insert(nbr, node_m.h + self.cost(node_m, nbr))
                 
        # Else node k is smaller than h (RAISE)
        else:
            for nbr in self.get_neighbors(node_m):
                if nbr.tag == 'NEW' or (nbr.parent == node_m and nbr.h != node_m.h + self.cost(node_m, nbr)):
                    nbr.parent = node_m
                    self.insert(nbr, node_m.h + self.cost(node_m, nbr))
                else:
                    if nbr.parent != node_m and nbr.h > node_m.h + self.cost(node_m, nbr):
                        self.insert(node_m, node_m.h)
                    else:
                        if nbr.parent != node_m and node_m.h > nbr.h + self.cost(nbr, node_m) and nbr.tag == 'CLOSED' and nbr.h > k:
    ...
    def prepare_repair(self, node):
        ...
        for nbr in self.get_neighbors(node):
            ...
            if nbr.is_dy_obs == True and nbr.is_obs == False:
                for node_nbr in self.get_neighbors(nbr):
                    # Modify the cost from this neighbor node to all this neighbor's neighbors using self.modify_cost
                    self.modify_cost(nbr,node_nbr)
        #### TODO END ####
